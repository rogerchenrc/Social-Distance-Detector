...
def detect(vid_input, vid_output, display):
    ...
    vs = cv2.VideoCapture(vid_input if vid_input else 0)
    writer = None
    (W, H) = (None, None)
    ...
    while True:
        ...
        if len(idxs) > 0:
            ...
            for i in idf:
                ...
                cv2.putText(frame, tot_str, (30, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
                cv2.putText(frame, high_str, (50, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 150), 1)
                cv2.putText(frame, low_str, (50, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 120, 255), 1)
                cv2.putText(frame, safe_str, (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)
        ...
        if vid_output != "" and writer is None:
            fourcc = cv2.VideoWriter_fourcc(*"MJPG")
            writer = cv2.VideoWriter(vid_output, fourcc, 30, (frame.shape[1], frame.shape[0]), True)

        if writer is not None:
            writer.write(frame)

    # sys.stdout.close()
    print("Process finished:{}".format(vid_output))
    writer.release()
    vs.release()
    cv2.destroyAllWindows()


# vid_input = "input/test2_oxford.mp4"
# vid_output = "output/v3test.avi"
# display = 1
# detect(vid_input, vid_output, display)